...
Here's a paragraph with a link to example.com.
...
You can reference map() and filter() inline, or use Array.from() with a Set<T>. For
async code, use await Promise.all([...]).
